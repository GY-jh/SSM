...
public class IOCByAnnotationTest {
     ...
     * @Service：将类标识为业务层组件
     * @Repository：将类标识为持久层组件 通过注解+扫描所配置的bean的id，默认值为类的小驼峰，即类名的首字母为小写的结果
     * 可以她刚刚标识组件的注解的value属性值设置bean的自定义id
     * @Autowired:实现自动装配功能的注解 1、@Autowired注解能够标识的位置
     ...
     * 且这些bean的id要赋值的属性的属性名都不一致，此时抛异常：NoUniqueBeanDefinitionException
     * d>此时可以在要赋值的属性上，添加一个注解@Qualifier
     * 通过该注解的value属性值，指定某个bean的id，将这个bean为属性赋值
     * <p>
     * 注意：若IOC容器中没有任何一个类型匹配的bean，此时抛出异常：NoSuchBeanDefinitionException
     * 在@Autowired注解有个属性required，默认值为true，要求必须完成自动装配
